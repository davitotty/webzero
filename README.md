...
The design constraint is simple: if memory can fragment, it will. If there are threads, they will deadlock. If there's a config parser, it will crash on edge cases.
WebZero eliminates all of those.
How It Works
1. The .web Bundle
Your site is compiled into a single binary file:
bashnode tools/wz.js build ./my-site
The bundle contains:
...
2. The Memory Model
cstatic Arena arena;  // lives in BSS — zero-initialized, 4MB
```
...
cstatic const char HDR_200_BR[] =
    ...
    "Content-Length: ";
4. Backpressure
cif (active_connections >= MAX_CONNS) {
...
Benchmarks
HardwareOSreq/seclatency p99Binary sizePentium III 1.0 GHzLinux 4.95,20018ms68 KBRaspberry Pi 1 BLinux 4.191,80052ms68 KBIntel Atom N270Linux 5.48,4009ms68 KBCore i5-6500Linux 5.1548,0001ms68 KBPentium 4 2.4GHzWindows XP SP33,10028ms112 KB
Benchmark: wrk -t4 -c50 -d30s, serving a 12KB HTML page (3.2KB brotli-compressed).
Quick Start
Option A — npm (recommended)
bashnpm install -g webzero
This installs the wz command globally and automatically downloads the right prebuilt binary for your platform.
bash# Build your site into a .web bundle
wz build ./my-site
...
bashgit clone https://github.com/davitotty/webzero
cd webzero
...
bash# Build the example landing page into a .web bundle
...
Inspect a Bundle
bashnode tools/wz.js inspect examples/landing-page.web
```
